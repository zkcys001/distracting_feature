...
def prepare_dirs(args):
    ...
    """
    if args.path_weight:
        if args.path_weight.startswith(args.log_dir):
            args.model_dir = args.path_weight
        else:
            if args.path_weight.startswith(args.regime):
                args.model_name = args.load_path
            else:
                args.model_name = "{}_{}".format(args.regime,args.path_weight)
    else:
        args.model_name = "{}_{}".format(args.regime, get_time())
    args.model_name=args.model_name+args.image_type
    if not hasattr(args, 'model_dir'):
        args.model_dir = os.path.join(args.log_dir, args.model_name)
    #args.data_path = os.path.join(args.data_dir, args.regime)
